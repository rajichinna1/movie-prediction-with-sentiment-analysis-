...
function show_details(movie_details,movie_title,my_api_key,movie_id,movie_title_org){
  ...
  /*fetchRecommendations(movie_title, my_api_key)
        .then(function(recommendations) {
          console.log('Recommendationssssssss:', recommendations);
          // Use recommendations here as needed
        })
        .catch(function(error) {
          console.error('Error fetching recommendations:', error);
        });*/
  recommendations=fetchRecommendations(movie_title,my_api_key);
  details = {
      'title':movie_title,
...
function fetchRecommendations(movie_title, my_api_key) {
    var rec_movies = [];
    var rec_posters = [];
    var rec_movies_org = [];
    var rec_year = [];
    ...
    $.ajax({
        type: 'GET',
        url: '/recommendations/' + movie_title,
        dataType: 'json',
        async: false, // Make the request synchronous
...
function fetchRecommendations(movie_title,my_api_key) {
  // Return a promise
  return new Promise(function(resolve, reject) {
    // Initialize arrays to store recommendation details
    var rec_movies = [];
    var rec_posters = [];
    var rec_movies_org = [];
    var rec_year = [];
    ...
    // Make the AJAX call to fetch recommendations
    $.ajax({
      type: 'GET',
      url: '/recommendations/' + movie_title,
      dataType: 'json',
      success: function(data) {
        // Handle the received recommendations data
        console.log('Recommendations:', data);

        // Iterate over recommendations and fetch movie details for each
        var promises = data.map(function(movieId) {
          return movie_details(movieId, my_api_key);
        });
        ...
        Promise.all(promises)
          .then(function(movieDetailsArray) {
            ...
            movieDetailsArray.forEach(function(movieDetails) {
              rec_movies.push(movieDetails.results[0].original_title);
              rec_movies_org.push(movieDetails.results[0].original_title);
              rec_year.push(new Date(movieDetails.results[0].release_date).getFullYear());
              rec_vote.push(movieDetails.results[0].vote_average);
            ...
            // Resolve the promise with recommendation details
            resolve({
              rec_movies: rec_movies,
              rec_movies_org: rec_movies_org,
              rec_posters: rec_posters,
      ...
      error: function(xhr, status, error) {
        // Reject the promise if there's an error with the AJAX call
        reject('Error fetching recommendations: ' + error);
      }
    });
  });
}*/
